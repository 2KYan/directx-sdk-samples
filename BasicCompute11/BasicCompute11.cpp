...
BufType g_vBuf0[NUM_ELEMENTS - 1];
BufType g_vBuf1[NUM_ELEMENTS - 1];
...
int __cdecl main()
{
    ...
    CreateStructuredBuffer(g_pDevice, sizeof(BufType), NUM_ELEMENTS - 1, &g_vBuf0[0], &g_pBuf0);
    CreateStructuredBuffer(g_pDevice, sizeof(BufType), NUM_ELEMENTS - 1, &g_vBuf1[0], &g_pBuf1);
    CreateStructuredBuffer(g_pDevice, sizeof(BufType), NUM_ELEMENTS, nullptr, &g_pBufResult);
#else
    CreateRawBuffer(g_pDevice, (NUM_ELEMENTS - 1) * sizeof(BufType), &g_vBuf0[0], &g_pBuf0);
    CreateRawBuffer(g_pDevice, (NUM_ELEMENTS - 1) * sizeof(BufType), &g_vBuf1[0], &g_pBuf1);
    CreateRawBuffer(g_pDevice, (NUM_ELEMENTS + 0) * sizeof(BufType), nullptr, &g_pBufResult);
#endif
    ...
    RunComputeShader(g_pContext, g_pCS, 2, aRViews, nullptr, nullptr, 0, g_pBufResultUAV, 4, 1, 1);
    printf("done\n");
    ...
    // Read back the result from GPU, verify its correctness against result computed by CPU
    {
        ...
        for (int i = 0; i < NUM_ELEMENTS; ++i) {
            printf("[%04d, %04d %04d] ", i, p[i].i, p[i].f);
            if (i && (i + 1) % 10 == 0) {
                printf("\n");
            }
        }
